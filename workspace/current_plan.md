# Current Weekly Plan

Week Start: 2026-06-08

## Monday: Push - Functional Hypertrophy
Goal: Chest, shoulders, and triceps with shoulder-safe pressing.
Warmup: Shoulder mobility, band pull-aparts, and light ramp sets.
- Bench press 3x8-10 @ RPE 7
- Incline dumbbell press 3x10-12 @ RPE 7
- Lateral raise 3x12-15
- Cable triceps pressdown 3x12-15
- Face pull 2x15-20
- Dead bug 3x8/side
Rest: Rest 2 minutes on presses and 60-90 seconds on accessories.
Pain modifications: If shoulder pain is 3/10 or higher, use neutral-grip dumbbells and skip overhead pressing.
Optional cardio: Optional 10-15 minutes easy incline walk.
Notes: Keep 1-2 reps in reserve on pressing.

## Tuesday: Pull - Back And Biceps
Goal: Upper back strength, pulling volume, and elbow-friendly arm work.
Warmup: Scapular pull-ups, light rows, and thoracic rotations.
- Lat pulldown 3x8-10 @ RPE 7
- Chest-supported row 3x10-12
- Single-arm cable row 2x10/side
- Face pull 3x12-15
- Dumbbell curl 3x10-12
- Side plank 3x30 sec/side
Rest: Rest 90-120 seconds on rows and pulldowns.
Pain modifications: Use straps or neutral grips if elbows or shoulders feel irritated.
Notes: Use controlled reps and avoid shrugging.

## Thursday: Legs - Strength Base
Goal: Lower-body strength with practical hypertrophy volume.
Warmup: Hip mobility, bodyweight squats, and glute activation.
- Squat 3x6-8 @ RPE 7
- Romanian deadlift 3x8-10
- Walking lunge 3x10/side
- Leg curl 3x10-12
- Calf raise 3x12-15
- Pallof press 3x10/side
Rest: Rest 2-3 minutes on squat and Romanian deadlift.
Pain modifications: If knees or low back feel off, reduce range and use leg press or split squat.
Notes: Stop sets if technique breaks down.

## Saturday: Upper + Core
Goal: Balanced upper-body work with core stability.
Warmup: Shoulder mobility and light push/pull supersets.
- Dumbbell bench press 3x8-10
- Seated cable row 3x10-12
- Landmine press 3x8/side
- Rear delt fly 3x12-15
- Cable crunch 3x10-12
- Farmer carry 3x30 meters
Rest: Rest 90 seconds between most sets.
Pain modifications: Replace landmine press with machine chest press if shoulder symptoms increase.
Optional cardio: Optional 20 minutes zone 2 cardio.
Notes: Keep this session moderate and clean.

## Sunday: Mobility + Core Recovery
Goal: Low-stress movement, trunk control, and recovery.
Warmup: Easy walk or bike for 5-10 minutes.
- Hip flexor stretch 2x45 sec/side
- Thoracic rotations 2x8/side
- Bird dog 3x8/side
- Pallof press 3x10/side
- Easy zone 2 cardio 30 minutes
Rest: Move calmly and keep breathing controlled.
Pain modifications: Skip any mobility drill that creates sharp pain.
Notes: This should feel restorative, not fatiguing.

## Generation Notes
Generated from the functional hypertrophy coaching model: practical volume, shoulder-aware pressing, frequent core work, and gradual progression.
